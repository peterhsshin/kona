...
bool NetlinkUtils::GetWifiGenerationCapabilitiesPerBand(
  ...
  *capa = {};
  ...
  if (band.HasAttribute(NL80211_BAND_ATTR_HT_CAPA)) {
    capa->ht_support = true;
  }

  if (band.HasAttribute(NL80211_BAND_ATTR_VHT_CAPA)) {
    capa->vht_support = true;
  }

  if (band.HasAttribute(NL80211_BAND_ATTR_IFTYPE_DATA)) {
    NL80211NestedAttr iftype_datas_attr(0);
    if (!band.GetAttribute(NL80211_BAND_ATTR_IFTYPE_DATA, &iftype_datas_attr)) {
      LOG(ERROR) << "Failed to get NL80211_BAND_ATTR_IFTYPE_DATA";
      return false;
    }

    vector<NL80211NestedAttr> iftype_datas;
    if (!iftype_datas_attr.GetListOfNestedAttributes(&iftype_datas)) {
      LOG(ERROR) << "Failed to get iftype_data's within NL80211_BAND_ATTR_IFTYPE_DATA";
      return false;
    }

    for (uint32_t index = 0; index < iftype_datas.size() ; index++) {
      if (iftype_datas[index].HasAttribute(NL80211_BAND_IFTYPE_ATTR_HE_CAP_MAC)) {
        NL80211NestedAttr iftypes_attr(0);

        if (!iftype_datas[index].GetAttribute(NL80211_BAND_IFTYPE_ATTR_IFTYPES, &iftypes_attr)) {
          LOG(ERROR) << "Failed to get NL80211_BAND_IFTYPE_ATTR_IFTYPES";
          return false;
        }

        if (iftypes_attr.HasAttribute(NL80211_IFTYPE_STATION)) {
          capa->sta_he_support = true;
        }

        if (iftypes_attr.HasAttribute(NL80211_IFTYPE_AP)) {
          capa->sap_he_support = true;
        }
      }
    }
  }
...
bool NetlinkUtils::ParseWifiGenerationInfoFromPacket(
    const NL80211Packet& packet,
  ...
  int current_band = -1;
  WifiGenerationCapabilities current_band_capa;

  // Make sure all flags are reset
  out_info->capa_2g = {};
  out_info->capa_5g = {};
  for (uint32_t band_index = 0; band_index < bands.size(); band_index++) {
    if (current_band == bands[band_index].GetAttributeId())
      continue;

    if (!NetlinkUtils::GetWifiGenerationCapabilitiesPerBand(bands[band_index],
                         &current_band_capa)) {
      return false;
    }

    current_band = bands[band_index].GetAttributeId();

    if (current_band == NL80211_BAND_2GHZ) {
      out_info->capa_2g = current_band_capa;
    } else if (current_band == NL80211_BAND_5GHZ) {
      out_info->capa_5g = current_band_capa;
    }
  }
...
bool NetlinkUtils::GetWifiGenerationInfo(
    uint32_t wiphy_index,
    WifiGenerationInfo* out_info) {
  NL80211Packet get_wiphy(
      netlink_manager_->GetFamilyId(),
  ...
  for (const auto& packet : packet_per_wiphy) {
    ...
    if (ParseWifiGenerationInfoFromPacket(packet, out_info)) {
      return true;
    }
  }
